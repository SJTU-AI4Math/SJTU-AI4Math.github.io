...
describe('GitHub Pages fallbacks', () => {
  it('adds directory indexes where static lecture paths collide with SPA routes', () => {
    ...
    expect(readFileSync(join(dist, 'snl/index.html'), 'utf8')).toBe(appShell)
    expect(readFileSync(join(dist, 'snl/documents/fulcrum-notes-snl/index.html'), 'utf8')).toBe(appShell)
    expect(readFileSync(join(dist, 'snl/documents/snl4gaokao/index.html'), 'utf8')).toBe(appShell)
